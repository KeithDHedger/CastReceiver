...
{
	variable_container_t *result=(variable_container_t*)malloc(sizeof(variable_container_t));
	...
	for (int i = 0; i < variable_num; ++i)
		result->values[i] = strdup(variable_init_values[i] ? variable_init_values[i] : "");
...
int VariableContainer_change(variable_container_t *object,int var_num, const char *value)
{
	if (value == NULL)
		value = "";
	...
	for (struct cb_list *it = object->callbacks; it; it = it->next)
		it->callback(it->userdata,var_num, object->variable_names[var_num],old_value, new_value);
...
{
	upnp_last_change_builder_t *result = (upnp_last_change_builder_t*)malloc(sizeof(upnp_last_change_builder_t));
	...
	if (builder->change_event_doc == NULL)
		{
			...
			struct xmlelement *toplevel=xmldoc_new_topelement(builder->change_event_doc,"Event",builder->xml_namespace);
...
upnp_last_change_collector_t *UPnPLastChangeCollector_new(variable_container_t *variable_container,const char *event_xml_namespace,struct upnp_device *upnp_device,const char *service_id)
{
	upnp_last_change_collector_t *result = (upnp_last_change_collector_t*)malloc(sizeof(upnp_last_change_collector_t));
	...
	VariableContainer_register_callback(variable_container,UPnPLastChangeCollector_callback,result);
	return(result);
}
...
void UPnPLastChangeCollector_notify(upnp_last_change_collector_t *obj)
{
	...
	if (VariableContainer_change(obj->variable_container,obj->last_change_variable_num,xml_doc_string))
		{
...
{
	upnp_last_change_collector_t *object=(upnp_last_change_collector_t*) userdata;
